...
/**
 * Acknowledge incident by monitor ID
 * POST /api/v1/incidents/:id/acknowledge
 */
export const acknowledgeIncidentV1 = async (req: Request, res: Response) => {
    try {
        const monitorId = (Array.isArray(req.params.id) ? req.params.id[0] : req.params.id) || '';
        const incident = await incidentService.acknowledgeByMonitorId(monitorId);

        if (!incident) {
            return res.status(404).json({ error: 'No open incident found for this monitor to acknowledge' });
        }

        res.status(200).json(incident);
    } catch (error: any) {
        console.error(`Error acknowledging incident for monitor ${req.params.id}:`, error);
        res.status(500).json({ error: error.message || 'Failed to acknowledge incident' });
    }
};

/**
 * Resolve incident by monitor ID
 * POST /api/v1/incidents/:id/resolve
 */
export const resolveIncidentV1 = async (req: Request, res: Response) => {
    try {
        const monitorId = (Array.isArray(req.params.id) ? req.params.id[0] : req.params.id) || '';
        const incident = await incidentService.resolveByMonitorId(monitorId);

        if (!incident) {
            return res.status(404).json({ error: 'No open incident found for this monitor to resolve' });
        }

        res.status(200).json(incident);
    } catch (error: any) {
        console.error(`Error resolving incident for monitor ${req.params.id}:`, error);
        res.status(500).json({ error: error.message || 'Failed to resolve incident' });
    }
};
